...
void nmrpowder(double *spc, double *f, mwSize M, double fmax, mwSize NOP, double A, double LB, double Kiso, double Kax, double Kasym, double KaxS)
{
  int i,j,g2,N,k,NN;
  double Fs, fc, fi, costh, sinth, cos2fi, maxi=-100000, UNI, c[3], KaxCurrent, U, V, X;
    ...
    for (k=0; k<NN; k++) {
        ...
        X = 2*sqrt(-2*log(U))*cos(6.2831853*V);
        ...
        for (i=0; i<N; i++) {
                ...
                fc = Kiso + KaxCurrent*(3*costh*costh-1)/2 + Kasym*sinth*sinth*cos2fi;
                for (j=0; j<NOP; j++)
...
{
  double *spc,*f;
  double fmax, A, LB, Kiso, Kax, Kasym, KaxS;
  ...
  /*  call the C subroutine*/ 
	nmrpowder(spc, f, N, fmax, NOP, A, LB, Kiso, Kax, Kasym, KaxS);
}
